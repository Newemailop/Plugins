...
async def activate_reply_raid(c: Client,m: Message):
    ...
    if m.reply_to_message_id:
        ...
        hell = await m.reply_text("Reply Raid Activating....")
        if u_id not in que:
            que.append(u_id)
            await hell.edit_text(f"Reply Raid has been activated on {username}")
        else:
            await hell.edit_text("You already have started reply raid for this user")
    else:
        try:
...
async def deactivate_reply_raid(c: Client, m: Message):
    ...
    if m.reply_to_message:
        ...
        hell = await m.reply_text("Reply Raid De-activating....")
        try:
            if u_id in que:
                que.remove(u_id)
                await hell.edit_text(f"Reply Raid has been De-activated on {username}")
                return
            await hell.edit_text("You haven't started reply raid for this user")
        except Exception:
            await hell.edit_text("You haven't activated reply raid for this user")
            return
